...
{
  // ultrasonic();
  colorWipe(1,strip.Color(random(0, 255), random(0, 255),random(0, 255) ), 1500 ,ultrasonic()/20000);
...
/*
  ...
  for (int block = 0; block < NUM_BLOCKS; block++) {
    pulseColor(block, strip.Color(25, 25, 25), 2, 12);
  }
  strip.clear();
*/
...
/*
  ...
  for (int block = 0; block < NUM_BLOCKS; block++) {
    theaterChaseRainbow(block, 0.5, 0);
  }
*/
...
void pulseColor(int block, uint32_t color, double duration, int steps) {
  for (int step = 0; step < steps; step++) {
...
void colorWipe(int block, uint32_t color, int wait, float brightness) {
...
void theaterChaseRainbow(int block, double wait, double cycles) {
...
float ultrasonic()
{
  ...
  //float ultrasonic = pulseIn(echoPin,HIGH);
